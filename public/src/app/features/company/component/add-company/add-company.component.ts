...
export class AddCompanyComponent {
  ...
  constructor(private fb: FormBuilder, private companyservice: CompanyService, private apiservice: BussinessService,
    private activate: ActivatedRoute, private _successMessage: MatSnackBar
  ...
  patchValue() {
    this.companyservice.getCompanyProfileById(this.Id).subscribe({
      next: (data: any) => {
  ...
  submitForm() {
    ...
    if (this.companyForm.valid) {
      ...
      if (this.Id <= 0) {
        ...
        this.companyservice.addCompanyProfile(request).subscribe({
          next: (data: any) => {
            if (data.success) {
        ...
        this.companyservice.updateCompanyProfile(request).subscribe({
          next: (data: any) => {
            if (data.success) {
              this.showSuccessMessage(data.message)
              this.formClose.emit(true)
