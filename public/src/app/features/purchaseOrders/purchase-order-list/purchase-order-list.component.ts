...
export class PurchaseOrderListComponent implements OnInit {
  ...
  updatePO(event: any): void {
    ...
    if (event.event.target.closest(".activityLogs-icon")) {
      const PurchaseOrderRowId = event.event.target.closest(".activityLogs-icon").getAttribute("data-id");
      this.tableRowId=PurchaseOrderRowId;
      this.isAuditlogOpen=true;
    }
  }
  ...
  renderActionIcons(params: any): string {
    return ` 
      ...
      <span class="icon-container text-secondary eye-icon" data-id="${params.data.poid}" title="View Order" style="display: block; width: 20px; height: 20px;">
